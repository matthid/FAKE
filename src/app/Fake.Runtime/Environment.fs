...
// Note: The same field is available in the old API, therefore we use a workaround to make the "new" api point to the old field.
let mutable internal monoArguments = ref ""

let mutable internal monoArgumentsHelper = LegacyApiHelper.ofRef monoArguments
let getMonoArguments, setMonoArguments = LegacyApiHelper.toGetSet monoArgumentsHelper
